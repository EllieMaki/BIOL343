...
positpal::start("Group_3_CC2.Rmd")
```
...
getwd()
cc1_data <- mydata[0:45, 0:8]
cc1_data$ovary_area <- (cc1_data$OvaryWidth.mm.*(cc1_data$OvaryHeight.mm.))
...
$$ A = W \times H$$
Where $A$ is Ovary Area in $mm^2$, $W$ is Ovary Width in mm, and $H$ is Ovary Height in mm 
...
area_means <- aggregate(ovary_area ~ PopulationType, data = cc1_data, mean)
```
...
kable(area_means, format = "html", caption = "Mean Ovary Area By Population Type")
```
...
```{r area-histogram, echo = FALSE, message = FALSE, warning = FALSE}
Plot1 <- ggplot(aes(x=ovary_area), data=cc1_data) + geom_histogram(binwidth=0.5) + xlab("Population Type") + ylab("Ovary Area (mm^2)") + labs(title="Ovary Area of Asexual and Sexual Solanum pimpinellifolium Populations") + theme_classic()
Plot1 + facet_wrap(.~PopulationType) 
```
...
![Image of *Solanum pimpinellifolium*](c:/Users/eliss/OneDrive - Queen's University/Intro to Stats/BIOL343/images/Solanum_pimpinellifolium.jpg) 

[Source:](https://littleredwagonnativenursery.com/products/everglades-tomato-solanum-pimpinellifolium) 
